...
void gui::render_gui(){
    ...
    ImGui::Begin("SceneWindow", NULL, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
    {
        ...
        parameters.scene_window.inner_img_pixel_width = windowSize.y * 1.78f;
        parameters.scene_window.inner_img_pixel_height = windowSize.y;

    
            
            // Define the position of the image inside the current scene window
            ImVec2 imagePos = ImVec2(
                windowPos.x + (windowSize.x - windowSize.y*1.78f) * 0.5,
                windowPos.y
            );
